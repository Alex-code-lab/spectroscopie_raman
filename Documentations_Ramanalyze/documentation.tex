...
% Encadrés
% ---------------------------------------------------------------------------
\usepackage{pifont}          % \ding{51} = ✓, \ding{114} = ☐
...
|-- spectra_plot.py       # Onglet Spectres : trace Plotly, export haute resolution
|-- analysis_tab.py       # Onglet Analyse : pics, ratios, fit sigmoide, export
|-- peak_selector.py      # Onglet Exploration : PCA correlee + PCA libre
|-- data_processing.py    # Chargement .txt, baseline, assemblage
|-- plotly_downloads.py   # Utilitaires export graphique (JS Plotly, printToPdf)
\end{lstlisting}
...
\paragraph{Gestion des colonnes (tubes).}
Un clic sur l'en-tête d'une colonne la sélectionne visuellement. Un clic
droit sur cet en-tête ouvre un menu contextuel proposant de \textbf{dupliquer}
ou de \textbf{supprimer} le tube correspondant. Des boutons dédiés dans la
barre d'outils permettent également d'ajouter un tube vierge en fin de
tableau.
...
\subsubsection{Feuille de protocole de paillasse}
\label{subsec:protocole}

Le bouton \textbf{Feuille de protocole\,\ldots} (onglet \textit{Métadonnées})
ouvre un dialogue interactif destiné à \textbf{tracer les opérations de pipetage}
sur la paillasse. Il affiche le tableau \texttt{df\_comp} sous forme d'une grille
où chaque ligne est un réactif et chaque colonne un tube expérimental.

\paragraph{Structure du tableau.}
Pour chaque tube, trois sous-colonnes sont disponibles :

\begin{itemize}
  \item \textbf{Volume (µL)} — lecture seule ; rappelle la quantité à pipeter.
  \item \textbf{Coordinateur \ding{114}} — case validée par le coordinateur.
  \item \textbf{Opérateur \ding{114}} — case validée par l'opérateur.
\end{itemize}

Une colonne \textbf{Vérification solution} permet en outre de confirmer
l'identité du flacon avant tout pipetage.

\paragraph{Mode guidé.}
En mode guidé, seule la cellule de l'étape courante est active ;
toutes les autres sont grisées et désactivées.
La séquence automatique suit l'ordre :

\begin{enumerate}
  \item Vérification de la solution du réactif courant.
  \item Tube~1 : confirmation du volume + case Coordinateur + case Opérateur.
  \item Tube~2, Tube~3, \ldots jusqu'au dernier tube.
  \item Passage au réactif suivant (Vérif → Tube~1 → Tube~2 → \ldots).
\end{enumerate}

Un libellé de statut vert indique l'étape en cours :
\texttt{→ Vérifier la solution : Solution~A} ou \texttt{→ Solution~A · Tube~1}.

\paragraph{Workflow coordinateur / opérateur.}
Le coordinateur coche sa case en premier (tube préparé), puis l'opérateur
valide après vérification indépendante.
Les boutons \textbf{Tout cocher — Coord.} et \textbf{Tout cocher — Opér.}
permettent de valider toutes les cases d'un rôle en un clic ;
\textbf{Tout décocher} remet tout à zéro.

\paragraph{Export : fichier unifié (4 feuilles).}
Le bouton \textbf{Exporter Excel\,\ldots} génère un fichier \texttt{.xlsx}
contenant les quatre feuilles suivantes :

\begin{itemize}
  \item \texttt{Protocole} — feuille visuelle mise en forme (en-têtes colorés,
        texte pivoté, symboles \ding{51}/\ding{114}).
  \item \texttt{Volumes} — données brutes de \texttt{df\_comp}.
  \item \texttt{Correspondance} — données brutes de \texttt{df\_map}.
  \item \texttt{EtatProtocole} — état de chaque case
        (\texttt{type}, \texttt{r\_idx}, \texttt{t\_idx}, \texttt{checked}).
\end{itemize}

\paragraph{Persistance et rechargement.}
\begin{itemize}
  \item Fermeture du dialogue : l'état des cases est conservé en mémoire.
  \item \textbf{Enregistrer les métadonnées\,\ldots} : la feuille
        \texttt{EtatProtocole} est incluse automatiquement dans le fichier
        sauvegardé.
  \item \textbf{Charger des métadonnées\,\ldots} : si le fichier contient
        une feuille \texttt{EtatProtocole}, toutes les cases reprennent
        exactement l'état sauvegardé.
\end{itemize}

\begin{remarque}
Récapitulatif du flux de travail en paillasse :
\begin{enumerate}
  \item Ouvrir \textbf{Feuille de protocole\,\ldots}.
  \item Suivre le mode guidé : Vérif → Coord → Opér, tube par tube.
  \item Cliquer \textbf{Exporter Excel\,\ldots} pour archiver.
  \item Recharger le fichier à tout moment via
        \textbf{Charger des métadonnées\,\ldots} pour reprendre où vous en
        étiez.
\end{enumerate}
\end{remarque}
...
\paragraph{Export du graphique.}
Le bouton \textbf{Exporter le graphique\,\ldots} ouvre un dialogue proposant :
\begin{itemize}
  \item \textbf{Format} : PNG (haute résolution, facteur $\times 2$), SVG
        (vectoriel) ou PDF (moteur Qt).
  \item \textbf{Taille de sortie} (PNG et SVG) : préréglages A4 paysage, A4
        portrait, A3 paysage, A3 portrait, écran large (1920$\times$1080),
        carré HD (1400$\times$1400), ou dimensions personnalisées en pixels.
\end{itemize}

L'export PNG et SVG utilise l'API JavaScript de Plotly directement depuis le
moteur de rendu de l'application (\texttt{Plotly.toImage} via
\texttt{QWebEnginePage.runJavaScript}). Le résultat est ainsi
\textbf{pixel-perfect} et identique à ce qui est affiché à l'écran.
L'export PDF utilise \texttt{QWebEnginePage.printToPdf} en format A4 paysage.

\subsection{Onglet Analyse}

\begin{enumerate}
  \item Rechargez le fichier combiné si nécessaire.
  \item Choisissez un \textbf{jeu de pics} :
    \begin{itemize}
      \item \textbf{532~nm — paires pré-enregistrées} : jeu de paires de pics
            optimisé pour le laser 532~nm.
      \item \textbf{785~nm} : pics à 412, 444, 471, 547, 1561~\cm.
      \item \textbf{Personnalisé} : saisissez librement les Raman Shifts en
            \cm, séparés par des virgules.
    \end{itemize}
  ...
  \item Cliquez \textbf{Analyser les pics} :
    \begin{itemize}
      ...
      \item calcul de tous les ratios $I_i / I_j$ pour chaque paire de pics
            $(i, j)$,
      \item jointure avec les métadonnées et calcul de la quantité de titrant
            $n(\text{titrant})$ en mol,
      \item affichage d'un tableau triable et d'un graphique interactif des
            ratios en fonction de la quantité de titrant.
    \end{itemize}
...
\paragraph{Fit sigmoïde et point d'équivalence.}
Pour chaque ratio calculé, il est possible d'ajuster une fonction sigmoïde
afin de localiser le point d'équivalence de la titration :
\begin{enumerate}
  \item Sélectionnez le ratio dans la liste \textbf{Ratio à ajuster}.
  \item Cliquez \textbf{Ajuster par sigmoïde} : le fit est réalisé par
        moindres carrés (\texttt{scipy.optimize.curve\_fit}) et la courbe
        ajustée est superposée au graphique.
  \item Cliquez \textbf{Afficher l'équivalence} pour annoter le graphique
        avec la valeur du point d'équivalence $x_\text{eq}$.
\end{enumerate}

\paragraph{Export du graphique (Analyse).}
Le bouton \textbf{Enregistrer le graphique} propose les mêmes options que
l'onglet Spectres : PNG haute résolution, SVG vectoriel ou PDF, avec choix
de la taille de sortie parmi les préréglages A4, A3, écran large, ou en
...
\paragraph{Indépendance des champs volume et concentration.}
Les champs $V_X$ et $C_X^\text{stock}$ sont \textbf{entièrement indépendants} :
modifier l'un n'affecte pas l'autre. Cela permet d'ajuster librement la
concentration du stock ou le volume pipetté sans contrainte automatique.
Le bouton \textbf{Quantités de matière\,\ldots} permet, si besoin, de
consulter ou de modifier directement la quantité de matière $n_X$ résultante
(cf.\ section~\ref{subsec:qmol}).
...
La case à cocher \textbf{Utilisation d'un compte-goutte} applique un
\textbf{preset} adapté au pipetage par compte-goutte. Après activation,
tous les champs restent librement modifiables : il s'agit uniquement d'un
point de départ commode, sans contrainte imposée.

Les valeurs pré-remplies par ce preset sont les suivantes :
...
Un bouton \textbf{Valeurs par défaut} dans le dialogue principal remet
l'ensemble des paramètres aux valeurs d'usine, sans fermer la fenêtre.
...
à \texttt{metadata\_model}. Contient :

\begin{itemize}
  \item Tableau des volumes (réactifs $\times$ tubes) avec éditeur interactif,
        gestion des colonnes par menu contextuel (ajout, suppression,
        duplication de tubes via clic droit sur l'en-tête de colonne).
  ...
  \item Dialogue de génération de volumes gaussiens
        (\texttt{GaussianVolumesDialog}) : paramètres principaux, solutions
        fixes C/D/E/F avec volumes et concentrations indépendants, preset
        compte-goutte, bouton \textbf{Valeurs par défaut}.
  \item Feuille de protocole de paillasse (\texttt{ProtocolDialog}) en mode
        libre et mode guidé, avec export Excel quatre feuilles.
...
\begin{itemize}
  ...
  \item Export haute résolution via \texttt{Plotly.toImage} (PNG $\times 2$,
        SVG vectoriel) et \texttt{QWebEnginePage.printToPdf} (PDF A4
        paysage), avec préréglages de taille (A4, A3, personnalisé).
\end{itemize}
...
\begin{itemize}
  ...
  \item Propose des jeux de pics prédéfinis (532~nm, 785~nm) et un mode
        personnalisé (saisie libre des Raman Shifts).
  \item Calcule l'intensité maximale dans la fenêtre $[\nu_0 \pm \Delta\nu]$
        pour chaque spectre et chaque pic (tolérance par défaut~: 5~\cm).
  \item Calcule tous les ratios $I_i / I_j$ pour chaque paire de pics $(i, j)$.
  \item Fusionne avec les métadonnées et calcule $n(\text{titrant})$ en mol.
  \item Graphique interactif des ratios en fonction de la quantité de titrant.
  \item Ajustement sigmoïde par ratio avec affichage du point d'équivalence
        $x_\text{eq}$ (\texttt{scipy.optimize.curve\_fit}).
  \item Export Excel avec deux feuilles : \texttt{intensites} et
        \texttt{ratios}.
  \item Export graphique haute résolution (PNG, SVG, PDF) via la même
        infrastructure que \texttt{spectra\_plot.py}.
\end{itemize}

\subsection{\texttt{plotly\_downloads.py}}

\begin{itemize}
  \item Utilitaires partagés pour l'export graphique.
  \item L'export PNG et SVG appelle \texttt{Plotly.toImage} via
        \texttt{QWebEnginePage.runJavaScript} : le moteur de rendu est
        identique à l'affichage interactif, sans recours à Kaleido.
  \item Un mécanisme de polling asynchrone (\texttt{QTimer}) attend la
        résolution de la promesse JavaScript avant d'écrire le fichier.
  \item L'export PDF utilise \texttt{QWebEnginePage.printToPdf}.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Fichier combiné} : CSV (\texttt{.csv}) ou Excel
        ...
        chemin est mémorisé.
  \item \textbf{Résultats d'analyse} : Excel (\texttt{.xlsx}), deux feuilles :
    \begin{itemize}
    ...
    \end{itemize}
  \item \textbf{Métadonnées et protocole} : Excel (\texttt{.xlsx}),
        quatre feuilles (\texttt{Protocole}, \texttt{Volumes},
        \texttt{Correspondance}, \texttt{EtatProtocole}).
  \item \textbf{Graphiques} (onglets Spectres et Analyse) : PNG haute
        résolution (facteur $\times 2$), SVG vectoriel ou PDF (A4 paysage),
        avec choix de la taille de sortie. L'export PNG et SVG utilise
        \texttt{Plotly.toImage} via JavaScript pour un rendu identique à
        l'affichage interactif.
  \item \textbf{Paires de pics} (onglet Exploration) : CSV exportable via
        le bouton \textbf{Exporter tableau CSV}.
\end{itemize}
...
\paragraph{L'export PDF ne correspond pas à l'affichage interactif.}
Le PDF est généré par le moteur d'impression Qt (\texttt{printToPdf}),
qui peut introduire de légères différences de mise en page selon le système.
Pour un rendu pixel-perfect identique à ce qui est affiché à l'écran,
préférez l'export \textbf{PNG haute résolution} ou \textbf{SVG vectoriel},
qui utilisent directement l'API JavaScript de Plotly.
