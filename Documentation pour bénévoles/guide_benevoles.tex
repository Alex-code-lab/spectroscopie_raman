...
\begin{titlepage}
  ...
  \begin{figure}[H]
    \centering
    {\setlength{\fboxsep}{0pt}\setlength{\fboxrule}{0.5pt}%
     \fbox{\includegraphics[width=1\linewidth]{Ramanalyze_visuel/Ramanalyze - ecran accueil .png}}}
...
\begin{tikzpicture}[node distance=0.45cm]
  ...
  \node[stepbox, fill=tab5col!55!wflight, below=of s5] (s6)
    {{\bfseries 6.}\enskip
     Générer le fichier combiné et l'enregistrer};

  \node[stepbox, fill=tab5col!45!wflight, below=of s6] (s7)
    {{\bfseries 7.}\enskip
     Remplir le tableau de correspondance spectres\,/\,tubes};

  \node[stepbox, fill=tab5col!36!wflight, below=of s7] (s8)
    {{\bfseries 8.}\enskip
     Enregistrer les métadonnées complètes (\texttt{.xlsx})};

  \node[stepbox, fill=tab5col!27!wflight, below=of s8] (s9)
    {{\bfseries 9.}\enskip
     Visualiser et vérifier les spectres (onglet \textbf{Spectres})};

  \node[stepbox, fill=tab5col!18!wflight, below=of s9] (s10)
    {{\bfseries 10.}\enskip
     Analyser les pics (onglet \textbf{Analyse})};

  \node[stepbox, fill=tab5col!9!wflight, below=of s10] (s11)
    {{\bfseries 11.}\enskip
     Ajuster la sigmoïde et lire le point d'équivalence};

  \node[stepbox, fill=wflight, below=of s11] (s12)
    {{\bfseries 12.}\enskip
  ...
  \draw[->, thick, color=tab5col!60!white] (s11.south) -- (s12.north);
