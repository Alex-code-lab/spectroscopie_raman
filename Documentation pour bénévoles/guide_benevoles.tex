...
\begin{titlepage}
  ...
  \begin{tikzpicture}
    \node[rounded corners=8pt, fill=mainblue, text=white,
          minimum width=13cm, minimum height=3.8cm,
          align=center] {
      {\Huge\bfseries Ramanalyze}\\[8pt]
      {\large Guide à l'usage des bénévoles}
    };
  \end{tikzpicture}
  ...
  \begin{tikzpicture}
    \node[draw=rulecolor, fill=figbg, rounded corners=7pt,
          minimum width=11cm, minimum height=5.5cm,
          align=center, text width=10cm,
          font=\small\color{darkgray}] {
      {\large\bfseries\color{rulecolor!70!darkgray}%
       \MakeUppercase{[ capture d'écran ]}}\\[8pt]
      Vue générale du logiciel ouvert,\\
      avec les cinq onglets visibles en haut de la fenêtre.
    };
  \end{tikzpicture}
...
Le laser est composé que d'une seule longueur d'onde, mais quand on lui fait traverser une solution, il interagit avec les molécules présentes, 
et certaines longueurs d'onde sont absorbées ou diffusées de manière spécifique : leur longueur d'onde change légèrement (et donc leur nombre d'onde qui n'est autre que 1/longueur d'onde).
En fonction de comment la lumière à été modifiée, on peut en déduire des informations sur les molécules présentes dans la solution.
C'est en quelque sorte une signature lumineuse propre à chaque molécule.

Chaque molécule produit ce signal à des valeurs précises, appelées
\textbf{pics}. En comparant l'intensité de deux pics, on calcule un
\textbf{ratio}. C'est ce ratio que l'on suit tout au long de l'expérience.
...
d'équivalence}. En connaissant la quantité ajoutée à ce moment-là, on peut
...
\begin{conseil}
  Inutile de comprendre la physique exacte du Raman pour utiliser ce logiciel. L'idéal est d'avoir compris notre objectif et comment techniquement on l'atteint (acquisition puis analyse).
...
\figph[Feuille de protocole Excel générée]{%
  Capture d'écran de la feuille de protocole ouverte dans Excel.
  Montrer les en-têtes colorés (bleu pour les volumes, vert pour la
  vérification), les cases à cocher et les noms de réactifs en
  colonne orange.}
...
\begin{conseil}
  ...
  et coche les cases au fur et à mesure.
\end{conseil}
...
\begin{enumerate}
  \item Cliquez sur \bouton{Ajouter des fichiers\ldots}.
        Une fenêtre de navigation s'ouvre.
  \item Naviguez jusqu'au dossier contenant les fichiers de votre
        expérience.
  ...
  \item Cliquez sur \bouton{Ouvrir} (ou \bouton{Ajouter} selon votre
        système).
\end{enumerate}

\figph[Sélection des fichiers dans la fenêtre de navigation]{%
  Capture d'écran de la fenêtre de navigation du système.
  Encadrer en rouge les fichiers sélectionnés (surlignés) et
  montrer l'extension \texttt{.txt} visible dans les noms de fichiers.
  Indiquer le bouton \og Ouvrir \fg{} en bas à droite.}
...
sélectionner, puis cliquez sur \bouton{Supprimer la sélection}.
...
\etape{4}{Générer le fichier combiné}

\begin{enumerate}
  \item Cliquez sur \bouton{Générer le fichier combiné\ldots}.
  \item Une fenêtre de sauvegarde s'ouvre.
  \item Choisissez un emplacement facilement retrouvable, par exemple
        le dossier de votre expérience.
  \item Donnez un nom explicite au fichier. Exemple :
        \texttt{GC001\_combined}.
  \item Cliquez sur \bouton{Enregistrer}.
\end{enumerate}

Un message de confirmation s'affiche. Le fichier combiné est créé.

\figbig{vis-fichier-raman-full.png}{Onglet Fichiers Raman après chargement complet — le message en bas confirme que le fichier combiné a bien été généré.}
...
\etape{5}{Revenir à l'onglet Métadonnées}

Cliquez sur l'onglet \onglet{Métadonnées}.
Le tableau de correspondance se trouve en bas de la page.

\etape{6}{Comprendre le tableau}
...
\etape{7}{Associer chaque spectre à son tube}
...
\etape{8}{Sauvegarder les métadonnées complètes}
...
\begin{enresume}
  À l'issue de cette étape, tous les spectres sont regroupés dans un
  fichier combiné unique et chaque spectre est associé à son tube.
...
\figph[Dialogue d'export du graphique]{%
  Capture d'écran du dialogue d'export avec le menu déroulant de
  format (PNG, SVG, PDF) et la liste de tailles préréglées.
  Indiquer par des flèches les trois étapes : choisir le format,
  choisir la taille, valider.}
...
\begin{tikzpicture}[node distance=0.45cm]
  ...
  \node[stepbox] (s1)
    {\numcercle{1}\enskip
     Renseigner les informations générales (onglet
     \textbf{\color{mainblue}Métadonnées})};
  ...
  \node[stepbox, below=of s3] (s4)
    {\numcercle{4}\enskip
     Charger les fichiers \texttt{.txt} (onglet
     \textbf{\color{mainblue}Fichiers Raman})} ;

  \node[stepbox, below=of s4] (s5)
    {\numcercle{5}\enskip
     Générer le fichier combiné et l'enregistrer};

  \node[stepbox, below=of s5] (s6)
    {\numcercle{6}\enskip
     Remplir le tableau de correspondance spectres\,/\,tubes};

  \node[stepbox, below=of s6] (s7)
    {\numcercle{7}\enskip
     Enregistrer les métadonnées complètes (\texttt{.xlsx})};

  \node[stepbox, below=of s7] (s8)
    {\numcercle{8}\enskip
     Visualiser et vérifier les spectres (onglet
     \textbf{\color{mainblue}Spectres})} ;

  \node[stepbox, below=of s8] (s9)
    {\numcercle{9}\enskip
     Analyser les pics (onglet
     \textbf{\color{mainblue}Analyse})} ;

  \node[stepbox, below=of s9] (s10)
    {\numcercle{10}\enskip
     Ajuster la sigmoïde et lire le point d'équivalence};

  \node[stepbox, below=of s10] (s11)
    {\numcercle{11}\enskip
     Exporter les résultats Excel et le graphique};

  \draw[->, thick, color=mainblue!50] (s1.south) -- (s2.north);
  \draw[->, thick, color=mainblue!50] (s2.south) -- (s3.north);
  \draw[->, thick, color=mainblue!50] (s3.south) -- (s4.north);
  \draw[->, thick, color=mainblue!50] (s4.south) -- (s5.north);
  \draw[->, thick, color=mainblue!50] (s5.south) -- (s6.north);
  \draw[->, thick, color=mainblue!50] (s6.south) -- (s7.north);
  \draw[->, thick, color=mainblue!50] (s7.south) -- (s8.north);
  \draw[->, thick, color=mainblue!50] (s8.south) -- (s9.north);
  \draw[->, thick, color=mainblue!50] (s9.south) -- (s10.north);
  \draw[->, thick, color=mainblue!50] (s10.south) -- (s11.north);
