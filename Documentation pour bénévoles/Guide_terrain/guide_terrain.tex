...
\definecolor{importantred}{RGB}{190, 35, 35}
...
\newtcolorbox{important}{
  enhanced, breakable,
  colback=importantred!7!white, colframe=importantred!85!black,
  left=7pt, right=7pt, top=5pt, bottom=5pt,
  arc=4pt, boxrule=0.9pt,
  title={\small\bfseries Important},
  fonttitle=\small\bfseries,
  attach boxed title to top left={yshift=-2pt, xshift=6pt},
  boxed title style={colback=importantred!85!black, colframe=importantred!85!black,
                     arc=3pt, boxrule=0pt},
}
...
\fancyfoot[C]{\small\color{darkgray}\textit{CitizenSers : Usage interne — Version 3.01, 05 mai 2026}}
...
\begin{titlepage}
  ...
  {\normalsize\bfseries\color{mainblue}
    Version 3.01 du logiciel et de la fiche terrain}\\[3pt]
  {\small\color{darkgray}Mise à jour : 05 mai 2026}
  ...
  {\color{darkgray}\small
    \textbf{CitizenSers} : Document à usage interne\\[4pt]
    Version 3.01 — 05 mai 2026\\[6pt]
    Pour toute question ou retour d'usage, contactez CitizenSERS / Bactérioxy.
  }
\end{titlepage}
...
\begin{attention}
  Ce guide correspond à la \textbf{version 3.01} du logiciel Ramanalyze
  et de la fiche terrain, datée du \textbf{05 mai 2026}. La fiche pourra
  évoluer selon les retours de terrain. Les bénévoles et les équipes
  concernées seront informé·es en cas de changement de version ou de
  procédure. Tout retour sur l'usage des outils est important pour
  améliorer les prochaines versions.
\end{attention}
...
\begin{center}
{\small
\begin{tabularx}{0.9\linewidth}{C{0.34\linewidth} C{0.05\linewidth} C{0.34\linewidth}}
  \toprule
  ...
  Noms générés automatiquement & & Noms retrouvés au chargement \\
...
Dans les deux cas, les champs les plus importants sont les
\textbf{personnes} et les \textbf{dates/heures} : ce sont eux qui
permettent de générer ou de retrouver automatiquement les noms.

\begin{important}
  Pour l'archivage, les fichiers doivent être enregistrés avec un
  nom précis. Toute fiche liée au prélèvement doit commencer par le
  \textbf{nom du prélèvement}, au format
  \texttt{PRE\_<initiales>\_AAAAMMJJ\_HHhMM}. Cela permet de trier
  les fichiers et de retrouver immédiatement l'échantillon concerné.
\end{important}
...
\begin{important}
  Ce même nom doit servir au nom du fichier principal :
  \texttt{PRE\_<initiales>\_AAAAMMJJ\_HHhMM.xlsx}. Les suffixes
  automatiques peuvent être conservés, mais le fichier doit toujours
  commencer par le nom du prélèvement.
\end{important}

\subsection{Le nom de la titration}

Si une titration du cuivre est réalisée, elle possède aussi son
propre nom. Il suit le format :

\begin{center}
  \large\texttt{<initiales operateur><initiales coordinateur>\_AAAAMMJJ\_HHhMM}
\end{center}

Il est généré à partir de :
\begin{itemize}
  \item l'\textbf{opérateur·ice} ;
  \item le ou la \textbf{coordinateur·ice} ;
  \item la \textbf{date} et l'\textbf{heure} de la titration.
\end{itemize}

\begin{attention}
  Si la titration n'a pas été faite, laissez simplement la case
  \champ{Titration du cuivre} sur \texttt{Non} ou décochée. Aucun nom
  de titration n'est nécessaire.
\end{attention}

\begin{important}
  Si une titration est réalisée, elle peut avoir un fichier séparé.
  Ce fichier de titration doit alors être enregistré avec le
  \textbf{nom de la manip de titration}, et non avec le nom
  \texttt{PRE\_...}. Ainsi, les fichiers de prélèvement et les fichiers
  de titration restent faciles à trier et à relier entre eux.
\end{important}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\linewidth]{../figures/onglet général - métadonnées - ramanalyze.png.png}
  \caption{Onglet Métadonnées ouvert — vue générale}
\end{figure}

\subsection{Charger une fiche existante (optionnel)}

Si une fiche a déjà été partiellement enregistrée, utilisez le bouton
en haut de l'onglet :

\begin{center}
  \bouton{Charger des métadonnées\ldots}
\end{center}

Sinon, continuez directement avec une fiche vierge.
...
Vérifiez qu'il correspond bien à votre session. Si le nom ne s'affiche
pas, vérifiez d'abord que les préleveur·ses, la date et l'heure sont
bien renseignés.
...
\begin{itemize}
  ...
  \item Le bouton \bouton{Carte / pointer\ldots} ouvre une carte
        interactive pour placer le point à la souris. En validant,
...
Renseignez :
\begin{itemize}
  \item \champ{Réalisé par} : prénom puis nom de la personne qui a fait le test.
...
% ─────────────────────────────────────────────────────────────
\subsection{Section 5 : Titration du cuivre}
% ─────────────────────────────────────────────────────────────

Si aucune titration du cuivre n'est réalisée, laissez la case
\champ{Titration du cuivre} décochée. La suite de la section reste
cachée et n'a pas à être remplie.

\begin{conseil}
  Lorsque la case \champ{Titration du cuivre} est cochée, de nouveaux
  onglets apparaissent dans le logiciel pour compléter cette partie
  spécifique : fichiers Raman, spectres, analyse et exploration. Les
  informations utiles au démarrage des mesures de terrain sont alors
  détaillées étape par étape, notamment le tableau des volumes, la
  correspondance spectres/tubes et la feuille de protocole.
\end{conseil}

\etape{12}{Renseigner la titration si elle est faite}

Si la titration est réalisée, cochez \champ{Titration du cuivre}, puis
renseignez :
\begin{itemize}
  \item \champ{Coordinateur·ice} : prénom puis nom ;
  \item \champ{Opérateur·ice} : prénom puis nom ;
  \item \champ{Date de la titration} et \champ{Heure de la titration} ;
  \item \champ{Lieu de la titration}.
\end{itemize}

Le \champ{Nom de la manip de titration} apparaît automatiquement à
partir de ces informations. Le bouton \bouton{Modifier} sert seulement
à saisir un nom manuel volontairement ; le bouton \bouton{Auto} revient
au nom généré.
...
\etape{13}{Enregistrer le fichier Excel}
...
\begin{important}
  Ne renommez pas le fichier avec un nom libre. Pour l'archivage,
  gardez le nom proposé ou un nom qui commence exactement par le
  \champ{Nom du prélèvement} (\texttt{PRE\_...}).
\end{important}

Le fichier \texttt{.xlsx} créé contient plusieurs feuilles :

\begin{center}
\begin{tabularx}{0.88\linewidth}{L{5.8cm} L{6.2cm}}
  \toprule
  ...
  \texttt{Titration-volumes} & Tableau de volumes (si titration) \\
  \texttt{Titration-correspondance} & Correspondance spectres / tubes (si titration) \\
  \texttt{Titration-EtatProtocole} & Suivi de la feuille de protocole (si titration) \\
  \bottomrule
\end{tabularx}
\end{center}
...
\begin{tabularx}{0.72\linewidth}{C{1.6cm} L{8cm}}
  ...
  \midrule
  \texttt{\_T} & Titration du cuivre réalisée ou prévue \\
...
Le logiciel Ramanalyze peut relire cette feuille si les libellés visibles
n'ont pas été renommés. Il est donc important de garder le format du
fichier, même si tous les champs ne sont pas remplis.

Les noms de prélèvement et de titration sont reconstruits à partir des
personnes et des dates/heures.

\begin{important}
  Si vous remplissez le modèle directement dans Excel, enregistrez le
  fichier principal avec le \textbf{nom du prélèvement} :
  \texttt{PRE\_<initiales>\_AAAAMMJJ\_HHhMM.xlsx}. Si une titration
  est réalisée dans un fichier séparé, ce fichier séparé doit porter
  le \textbf{nom de la manip de titration}.
\end{important}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{../figures/modeèle excel 2026 05 05.png}
  \caption{Feuille Mesures du fichier Excel modèle}
\end{figure}
...
\begin{center}
{\small
\begin{tabularx}{0.96\linewidth}{C{1.3cm} C{1.7cm} L{3.6cm} X}
  \toprule
  \textbf{Ligne} & \textbf{Colonne} & \textbf{Champ} & \textbf{Ce qu'il faut saisir} \\
  \midrule
  3 & B & Nom du prélèvement & Automatique si préleveur·ses + date/heure sont saisis ; sinon format \texttt{PRE\_...} \\
...
\begin{center}
{\small
\begin{tabularx}{0.88\linewidth}{C{1.6cm} C{1.6cm} X}
  \toprule
  \textbf{Colonne} & & \textbf{Ce qu'il faut saisir} \\
  \midrule
  A & (nom du champ) & \texttt{Préleveur·se 1}, \texttt{Préleveur·se 2}\ldots \\
  B & & Prénom puis nom de la personne \\
  C & (nom du champ) & \texttt{Association} \\
...
\begin{conseil}
  ...
  le format de la ligne 8 (noms des champs en A et C, valeurs en B et D).
\end{conseil}
...
\begin{center}
{\small
\begin{tabularx}{0.9\linewidth}{L{5cm} C{2cm} X}
  \toprule
...
autres mesures, titration) contiennent chacune une ligne avec les noms
des champs, puis une ligne pour les valeurs à saisir.
...
\begin{longtable}{L{4.8cm} L{2.8cm} L{5.5cm}}
  ...
  Test ammonium réalisé par & --- & Prénom puis nom \\
  ...
  Opérateur·ice & --- & Prénom puis nom \\
  Coordinateur·ice & --- & Prénom puis nom \\
  Date de la titration & JJ/MM/AAAA & \\
  Heure de la titration & HH:MM & \\
  Lieu de la titration & --- & \\
  Nom de la manip de titration & --- & Généré à partir opérateur·ice + coordinateur·ice + date/heure \\
  \midrule
...
\begin{attention}
  Dans Excel, ne commencez pas par inventer les noms. Renseignez d'abord
  les préleveur·ses, l'opérateur·ice, le ou la coordinateur·ice, et les
  dates/heures. Ramanalyze s'appuie sur ces champs pour retrouver les
  noms automatiquement au chargement.
\end{attention}

\begin{important}
  Avant de transmettre ou d'archiver le classeur Excel, vérifiez le
  nom du fichier. Le classeur de prélèvement doit être enregistré sous
  le nom \texttt{PRE\_...}. Le fichier séparé de titration, s'il existe,
  doit porter le nom propre de l'expérience de titration.
\end{important}
...
\begin{tabularx}{0.88\linewidth}{C{0.8cm} L{10.5cm}}
  ...
  $\Box$ & Nom du prélèvement généré ou reconstructible au format \texttt{PRE\_...} \\
  ...
  $\Box$ & Titration renseignée avec opérateur·ice, coordinateur·ice, date, heure et lieu (si réalisée) \\
  $\Box$ & Fichier principal enregistré avec un nom commençant par \texttt{PRE\_...} \\
  $\Box$ & Fichier séparé de titration nommé avec le nom de la manip de titration (si concerné) \\
  \bottomrule
\end{tabularx}
\end{center}
...
\begin{enresume}
  \begin{itemize}
    ...
    \item En cas de question sur le format, la procédure ou l'usage des
          outils, contactez CitizenSERS / Bactérioxy.
    \item Tout retour d'usage est important, même s'il concerne un détail :
          il permet d'améliorer les prochaines versions.
  \end{itemize}
\end{enresume}
